...
app_install_explorer()
{
    ...
    cp "$DEPLOY_PATH/config/explorer-server.js" "$EXPLORER_PATH/server.js"
    echo "{\"title\": \"$CHAIN_NAME Explorer\", \"server\": \"http:\/\/$NODE_IP:$NODE_PORT\/api\", \"alias\": \"Bridgechain\", \"activeDelegates\": \"$FORGERS\", \"rewardOffset\": 0, \"currencies\": [], \"knownWallets\": {}, \"defaults\": {\"currency\": null}, \"config\": {\"priceChart\": false}}" > "$EXPLORER_PATH/networks/chainid.json"
    mv "$EXPLORER_PATH/package.json" "$EXPLORER_PATH/package.orig.json"
    jq ".scripts.chainid = \"npm run dev -- --env.network=chainid --env.host=$EXPLORER_IP --env.port=$EXPLORER_PORT\"" "$EXPLORER_PATH/package.orig.json" > "$EXPLORER_PATH/package.json"
    HOST="$EXPLORER_IP" PORT="$EXPLORER_PORT" node "$EXPLORER_PATH/build/build.js" --network chainid
