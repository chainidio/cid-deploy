...
app_install_node()
{
    ...
    sed -i -e "s/var db_name = \"cid_\" + network_name;/var db_name = \"$DATABASE_NAME\";/g" createGenesisBlock.js
    ...
    sed -i -e "s/var totalpremine = 210000000000000000;/var totalpremine = $TOTAL_PREMINE;/g" createGenesisBlock.js
    sed -i -e "s/4100/$NODE_PORT/g" createGenesisBlock.js
    sed -i -e "s/send: 00000000/send: $FEE_SEND/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/vote: 000000000/vote: $FEE_VOTE/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/secondsignature: 000000000/secondsignature: $FEE_SECOND_PASSPHRASE/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/delegate: 0000000000/delegate: $FEE_DELEGATE/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/multisignature: 000000000/multisignature: $FEE_MULTISIG/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    ...
    sed -i -e "s/blocktime: 2/blocktime: $BLOCK_TIME/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/maxTxsPerBlock: 50/maxTxsPerBlock: $TXS_PER_BLOCK/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/offset: 7600/offset: $REWARD_HEIGHT_START/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/500000000, \/\//$REWARD_PER_BLOCK, \/\//g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/500000000 \/\//$REWARD_PER_BLOCK \/\//g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    sed -i -e "s/totalAmount: 1250000000000000000,/totalAmount: $MAX_TOKENS_PER_ACCOUNT,/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    if [[ "$UPDATE_EPOCH" == "Y" ]]; then
        sed -i -e "s/epochTime: new Date(Date.UTC(2018, 5, 21, 13, 0, 0, 0))/epochTime: new Date(Date.UTC($YEAR, $MONTH, $DAY, $HOUR, $MINUTE, $SECOND, 0))/g" "$BRIDGECHAIN_PATH/helpers/constants.js"
    fi
